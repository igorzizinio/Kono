    ...
    override suspend fun run(event: ChatInputCommandInteractionCreateEvent) {
        ...
        when (val preview = upgradeCharacterHandler.previewActiveCharacter(discordId)) {
            is UpgradeCharacterHandler.PreviewResult.Ready -> {
                ...
                when (val upgradeResult = upgradeCharacterHandler.executeActiveCharacter(discordId)) {
                    is UpgradeCharacterHandler.Result.Success ->
                        update.edit {
                            content = "✅ **${upgradeResult.characterName}** chegou ao **Lv.${upgradeResult.newLevel}**! " +
                    ...
                    is UpgradeCharacterHandler.Result.InvalidCardType ->
                        update.edit { content = "❌ A carta ativa não é um personagem válido."; components = mutableListOf() }
                    is UpgradeCharacterHandler.Result.MaxLevelReached ->
                        update.edit {
                            content = "⛔ Seu personagem já está no nível máximo (${upgradeResult.currentLevel}/${upgradeResult.levelCap})."
                            components = mutableListOf()
                        }
                    is UpgradeCharacterHandler.Result.NotEnoughKonos ->
                        update.edit {
                            content = "❌ KonoCoins insuficientes: precisa de **${upgradeResult.required} ₭**, você tem **${upgradeResult.current} ₭**."
                            components = mutableListOf()
                        }
                    is UpgradeCharacterHandler.Result.NotEnoughCopies ->
                        update.edit {
                            content = "❌ Cópias insuficientes: precisa de **${upgradeResult.required}**, você tem **${upgradeResult.current}**."
                            components = mutableListOf()
                        }
                    is UpgradeCharacterHandler.Result.PersistFailed ->
                        update.edit { content = "⚠️ Erro ao salvar o upgrade. Tente novamente."; components = mutableListOf() }
                }
            }
            ...
            is UpgradeCharacterHandler.PreviewResult.NotEnoughKonos ->
                deferred.respond {
                    content = "❌ KonoCoins insuficientes: precisa de **${preview.required} ₭**, você tem **${preview.current} ₭**."
                }
            is UpgradeCharacterHandler.PreviewResult.NotEnoughCopies ->
                deferred.respond {
                    content = "❌ Cópias insuficientes: precisa de **${preview.required}**, você tem **${preview.current}**."
                }
        }
    }
}
